...
const decoy_marker = [100.77940, 13.73059]
...
const InfoTech = new longdo.Marker({ lon:InfoTech_loc[0], lat: InfoTech_loc[1]},
    {
        ...
        icon: {
            url : 'https://cdn.discordapp.com/attachments/987596328244772877/990288966660022282/Red.png',
        }
    });
...
function init() {
    ...
    map.location({lon:decoy_marker[0],lat:decoy_marker[1]});
    /*Set map layers*/
    map.Layers.setBase(longdo.Layers.GRAY);
    map.Layers.add(longdo.Layers.TRAFFIC);
    result = map.location();
    console.log(result);
    map.Overlays.add(InfoTech);
    map.Overlays.add(AgriTech);
    map.Overlays.add(Engineer);
    map.Overlays.add(FoodIndus);
    map.Overlays.add(OffPres);
    map.Overlays.add(LitArts);


};

function markerOnCrosshair() {
    var newMarker = new longdo.Marker({lon:decoy_marker[0], lat:decoy_marker[1]},
        {
            title: 'นี่คือคุณ',
            icon: {
    ...
    map.Route.add({lon:decoy_marker[0], lat:decoy_marker[1]});
